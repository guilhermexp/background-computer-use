...
3. **Actions without an explicit `cursor` render a hardcoded "Codex" overlay.** `CursorCoordinator.swift:99` falls back to `CursorProfile.codex` (`CursorModels.swift:9-13`). On a multi-agent machine this looks like a second, unknown agent is acting. Overlay should be opt-in: no `cursor` in the request, no visual cursor.
...
- **cursor-overlay**: remove the `CursorProfile.codex` fallback for HTTP actions; when a request has no `cursor` object, no visual cursor session is created or animated. Behavior with an explicit `cursor` is unchanged.
...
- **BREAKING (minor):** clients that send extra/unknown fields will start receiving `invalid_request` instead of silent acceptance; clients relying on the implicit "Codex" cursor must now pass `cursor` explicitly. Both breaks are intentional and safer for agent use. `/v1/routes` docs must reflect the new behavior.
