...
3. **Actions without an explicit `cursor` render a hardcoded "Codex" overlay.** `CursorCoordinator.swift:99` falls back to `CursorProfile.codex` (`CursorModels.swift:9-13`). On a multi-agent machine this looks like a second, unknown agent is acting. The default cursor should instead be an explicit BCU identity that is reused whenever the request omits `cursor`.
...
- **cursor-overlay**: replace the `CursorProfile.codex` fallback for HTTP actions with a stable default BCU cursor (`id: "bcu"`, `name: "BCU"`); when a request has no `cursor` object, the runtime animates that same default session. Behavior with an explicit `cursor` is unchanged.
