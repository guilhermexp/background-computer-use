...
- [x] 2.1 Teste vermelho: nó de estado não contém locator duplicado com `ancestorFingerprints`/`rolePath`; `nodeID`, `refetchFingerprint` e `displayIndex` presentes.
- [x] 2.2 Teste vermelho de orçamento: custo serializado por nó abaixo do teto declarado, com a medição no texto da falha.
- [x] 2.3 Remover `.prettyPrinted` e `.sortedKeys` do encoder em `Shared/JSONSupport.swift`.
- [x] 2.4 Deixar um único locator canônico no DTO de nó em `StatePipelineExperiment.makeSurfaceNodeDTO`; remover as cópias aninhadas da resposta.
- [x] 2.5 Confirmar que os quatro `target.kind` resolvem inalterados após o trim (teste de round-trip leitura → ação).

## 3. Identidade de DOM e consulta dirigida (P3)

- [x] 3.1 Teste vermelho: nó de elemento HTML com `id` expõe `domIdentifier`; nó sem valor omite o campo.
- [x] 3.2 Acrescentar `AXDOMIdentifier` à allowlist `AXAttributeNames` e ao caminho de captura; expor como `domIdentifier` no DTO de nó.
- [x] 3.3 Teste vermelho: `find_elements` por papel + substring devolve só os nós casados, com `stateToken` e `interactionToken` da mesma leitura; query sem match devolve lista vazia com sucesso.
- [x] 3.4 Implementar `Actions/FindElements/FindElementsRouteService.swift` (read-only) reaproveitando a projeção existente.
- [x] 3.5 Wiring da rota (5 pontos): `RouteRegistry` (enum + descriptor + schemas), `RuntimeServices`, `Router`, facade `BackgroundComputerUseRuntime`, `Contracts/FindElementsContracts.swift`.
- [x] 3.6 Teste de contrato: campos de request/response documentados em `/v1/routes` batem com os DTOs reais.
