...
struct WindowResizeRouteService {
    ...
    func resize(request: ResizeRequest) throws -> ResizeResponse {
        ...
        let warnings = [
            "Resize targets desktop-global AppKit coordinates and derives the final window frame from one named handle.",
        ]
        ...
        guard request.toX.isFinite, request.toY.isFinite else {
            ...
            return ResizeResponse(
                ...
                ok: false,
                cursor: cursor,
                action: ResizeActionDTO(
                    kind: "resize",
                    requested: ResizeRequestedDTO(
                ...
                error: ActionErrorDTO(
                    ...
                    message: "The supplied resize destination must be finite AppKit-global coordinates."
                )
            )
        }
        ...
        let outcome = planner.plan(
            ...
            directive: .resize(
                handle: request.handle,
                destination: CGPoint(x: request.toX, y: request.toY)
            ),
        ...
        case .noop(let plan):
            ...
            noopWarnings.append("The supplied resize destination matched the current handle position closely; the runtime treated this as a no-op.")
            return ResizeResponse(
                contractVersion: ContractVersion.current,
                ok: true,
                cursor: cursor,
        ...
        case .executable(let plan):
            ...
            return ResizeResponse(
                ...
                error: execution.effectVerified ? nil : ActionErrorDTO(
                    ...
                    message: "The runtime projected the resize path, but the live window frame did not converge on the expected destination."
                )
            )
        }
    }
