...
enum RuntimePermissionInstructions {
    static func make(permissions: RuntimePermissionsDTO, baseURL: URL?) -> BootstrapInstructionsDTO {
        ...
        var agent = [
            ...
            "Action routes show the default bcu cursor session when cursor is omitted. Pass cursor.id only when you need a separate continuous on-screen cursor lane."
        ]
        if let baseURL {
