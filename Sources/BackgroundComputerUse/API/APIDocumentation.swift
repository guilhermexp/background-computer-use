...
enum APIDocumentation {
    static let guide = APIGuideDTO(
        ...
        concepts: [
            ...
            APIConceptDTO(
                name: "verification",
                description: "Click routes never treat dispatch as proof. A click is classification=success only when the transport dispatched AND verification.intentSignals is non-empty. Intent signals are target-local or structural: target_region_changed (targetRegionChangeRatio at or above targetRegionChangeThreshold), ocr_anchor_disappeared, focused_element_changed, modal_dialog_opened, window_title_changed (native surfaces only), target_state_changed, and web_area_text_changed (web renderer only, after two identical pre-dispatch web-area text samples and a differing post-settle sample). Whole-window rendered-text and selection-summary changes remain ambient noise on every surface; a web-renderer window-title change is ambient too. webAreaBaselineStable and webAreaBaselineDiagnostic expose whether the scoped baseline and post-settle comparison were usable; missing or unstable evidence never earns intent credit. Every click route uses this same gate. Other mutating routes still carry their own per-route verification blocks.",
                fields: nil
            ),
            APIConceptDTO(
                name: "imageMode",
